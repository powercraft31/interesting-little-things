...
async function runDailyBilling(pool: Pool): Promise<void> {
  try {
    // 昨天的日期
    ...
    for (const row of result.rows) {
      if (!byAsset[row.asset_id]) {
        byAsset[row.asset_id] = { org_id: row.org_id, arbitrage_profit: 0, savings: 0 };
      }
      // B-side: discharge kWh × PLD (R$/MWh ÷ 1000 = R$/kWh)
      byAsset[row.asset_id].arbitrage_profit += Number(row.total_discharge_kwh) * (Number(row.avg_pld) / 1000);
      // C-side: discharge kWh × retail rate
      byAsset[row.asset_id].savings += Number(row.total_discharge_kwh) * Number(row.retail_buy_rate_kwh);
    }
    ...
    for (const [assetId, data] of Object.entries(byAsset)) {
      ...
      await pool.query(`
        INSERT INTO revenue_daily
          (asset_id, org_id, date, arbitrage_profit_reais, savings_reais, revenue_reais, cost_reais, profit_reais)
        VALUES ($1, $2, $3, $4, $5, $4, 0, $4)
      ...
      `, [assetId, data.org_id, dateStr, arbitrage, savings]);
    }

    console.log(`[BillingJob] Settled revenue for ${Object.keys(byAsset).length} assets on ${dateStr}`);
  } catch (err) {
