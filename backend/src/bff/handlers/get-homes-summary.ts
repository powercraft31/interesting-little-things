...
/**
 * GET /api/homes/summary
 * EP-8: Cross-home comparison table.
  ...
  const { rows } = await queryWithOrg(
    `SELECT
     ...
     LEFT JOIN LATERAL (
       ...
       FROM assets a3
       WHERE a3.home_id = h.home_id AND a3.is_active = true AND a3.asset_type = 'INVERTER_BATTERY'
       LIMIT 1
     ) a2 ON true
     GROUP BY h.home_id, h.name, a2.operation_mode
     ORDER BY h.name`,
    [dateParam],
    rlsOrgId,
  );
