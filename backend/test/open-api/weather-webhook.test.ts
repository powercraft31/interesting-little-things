...
describe("weather-webhook handler (M7 inbound)", () => {
  ...
  afterAll(async () => {
    await pool.query(`DELETE FROM weather_cache WHERE location = 'TEST_LOC'`);
    await closeAllPools();
  });

  it("should return 401 when secret is missing", async () => {
    const res = makeRes();
    await handleWeatherWebhook(
      makeReq(testPayload) as Request,
      res as Response,
    );
  ...
  it("should write to weather_cache and return 200 with valid payload", async () => {
    const res = makeRes();
    await handleWeatherWebhook(
      makeReq(testPayload, "test-secret-2026") as Request,
      res as Response,
    );
  ...
  it("should overwrite on duplicate push (UPSERT idempotency)", async () => {
    ...
    const res = makeRes();
    await handleWeatherWebhook(
      makeReq(updated, "test-secret-2026") as Request,
      res as Response,
    );
