...
describe("ccee-webhook handler (M7 inbound)", () => {
  ...
  afterAll(async () => {
    // Clean up test data
    await pool.query(`DELETE FROM pld_horario WHERE mes_referencia = 202699`);
    await closeAllPools();
  });
  ...
  it("should write to pld_horario and return 200 with valid payload", async () => {
    ...
    expect(parseFloat(result.rows[0].pld_hora)).toBe(450.0);
  });

  it("should overwrite on duplicate push (UPSERT idempotency)", async () => {
    const updated = { ...testPayload, price_brl_mwh: 510.0 };
    ...
    expect(result.rows).toHaveLength(1);
    expect(parseFloat(result.rows[0].pld_hora)).toBe(510.0);
  });
});
