...
process.env.QUEUE_URL = 'https://sqs.us-east-1.amazonaws.com/123456/timeout-queue';
...
function makeDRCommandEvent(overrides: Partial<{
  dispatchId: string;
  assetId: string;
  targetMode: string;
  orgId: string;
...
describe('dispatch-command handler', () => {
  ...
  it('MQTT failure: marks dispatch as FAILED via DynamoDB UpdateCommand', async () => {
    ...
    await expect(handler(makeDRCommandEvent())).rejects.toThrow('MQTT connection lost');
  ...
  it('DynamoDB PutCommand failure: throws immediately, no MQTT or SQS calls', async () => {
    ...
    await expect(handler(makeDRCommandEvent())).rejects.toThrow('DynamoDB throttled');
  ...
  it('SQS failure: throws error (DB and MQTT already succeeded)', async () => {
    ...
    await expect(handler(makeDRCommandEvent())).rejects.toThrow('SQS unavailable');
