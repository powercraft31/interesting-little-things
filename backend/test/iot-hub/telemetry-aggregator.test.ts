...
describe("telemetry-aggregator (hourly rollup — v5.15 sourced from asset_5min_metrics)", () => {
  ...
  it("queries asset_5min_metrics and UPSERTs into asset_hourly_metrics (no factor, values sum directly)", async () => {
    // First call: SELECT from asset_5min_metrics (v5.15: sum directly, no factor)
    ...
    // v5.15: source is asset_5min_metrics, NOT telemetry_history
    expect(selectCall[0]).toContain("FROM asset_5min_metrics");
    expect(selectCall[0]).toContain("SUM(bat_charge_kwh)");
    expect(selectCall[0]).toContain("SUM(bat_discharge_kwh)");
    expect(selectCall[0]).toContain("SUM(pv_energy_kwh)");
    expect(selectCall[0]).toContain("SUM(grid_import_kwh)");
    expect(selectCall[0]).toContain("SUM(grid_export_kwh)");
    expect(selectCall[0]).toContain("SUM(load_kwh)");
    // v5.15: no factor applied — values are already in kWh
    expect(selectCall[0]).not.toContain("1.0/4");
    expect(selectCall[0]).not.toContain("1.0/12");

    // First UPSERT: ASSET_SP_001 — verify all params
  ...
  it("handles empty asset_5min_metrics gracefully (no assets to aggregate)", async () => {
