# 10. Database Schema — v5.7
> **版本**: v5.7 | **建立日期**: 2026-02-28 | **負責人**: Shared Infrastructure
>
> **變更說明**: v5.7 External Ingestion — pld_horario 改為由 M7 Inbound Webhook 動態更新；
> weather_cache 啟用 M7 Webhook 寫入；dispatch_commands 表（v5.6 已建）補記文件。
> 表格總數維持 18。
...
-- ⚠️ v5.7 說明：此表自 v5.7 起由 M7 Inbound Webhook (POST /webhooks/weather) 動態接收天氣資料。
...
-- ⚠️ v5.7 說明：自 v5.7 起，此表由 M7 Inbound Webhook (POST /webhooks/ccee-pld) 動態更新。
