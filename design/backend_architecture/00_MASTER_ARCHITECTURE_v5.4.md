...
| # | 文件名 | 路徑 | 說明 |
|---|--------|------|------|
| 00 | **MASTER_ARCHITECTURE** | `00_MASTER_ARCHITECTURE_v5.4.md` | 系統總控藍圖（本文件） |
| M1 | **IOT_HUB_MODULE** | [01_IOT_HUB_MODULE_v5.3.md](./01_IOT_HUB_MODULE_v5.3.md) | IoT Hub — MQTT 接入、動態解析器、Asset Shadow |
| M2 | **OPTIMIZATION_ENGINE_MODULE** | [02_OPTIMIZATION_ENGINE_MODULE_v5.2.md](./02_OPTIMIZATION_ENGINE_MODULE_v5.2.md) | Optimization Engine — 4 種策略演算法、排程最佳化 |
| M3 | **DR_DISPATCHER_MODULE** | [03_DR_DISPATCHER_MODULE_v5.2.md](./03_DR_DISPATCHER_MODULE_v5.2.md) | DR Dispatcher — 調度指令、SQS 逾時、狀態追蹤 |
| M4 | **MARKET_BILLING_MODULE** | [04_MARKET_BILLING_MODULE_v5.2.md](./04_MARKET_BILLING_MODULE_v5.2.md) | Market & Billing — 電價計費、收益計算、PostgreSQL schema |
| M5 | **BFF_MODULE** | [05_BFF_MODULE_v5.3.md](./05_BFF_MODULE_v5.3.md) | Frontend BFF — 聚合 API、Cognito 授權、中間件鏈 |
...
| 09 | **SHARED_LAYER** | [09_SHARED_LAYER_v5.4.md](./09_SHARED_LAYER_v5.4.md) | 公共型別定義、API 契約、EventSchema、目錄結構 |
| 10 | **DATABASE_SCHEMA** | [10_DATABASE_SCHEMA_v5.4.md](./10_DATABASE_SCHEMA_v5.4.md) | PostgreSQL 完整 DDL — 15 張表、ER 圖、Migration 管理 |
...
所有模組的輸入規格（Request Schema）與輸出規格（Response / Event Payload），均定義於 [`09_SHARED_LAYER_v5.4.md`](./09_SHARED_LAYER_v5.4.md)，一旦版本發佈，視同**鎖死的硬體腳位**，不得單方面修改。
...
|---------|---------|---------|------|---------|
| Shared | Shared Layer | **v5.4** | [09_SHARED_LAYER](./09_SHARED_LAYER_v5.4.md) | 公共型別、Connection Pool |
| Shared | Database Schema | **v5.4** | [10_DATABASE_SCHEMA](./10_DATABASE_SCHEMA_v5.4.md) | PostgreSQL — 15 張表 |
| M1 | IoT Hub | **v5.3** | [01_IOT_HUB](./01_IOT_HUB_MODULE_v5.3.md) | Lambda + IoT Core + DynamoDB |
| M2 | Optimization Engine | v5.2 | [02_OPTIMIZATION_ENGINE](./02_OPTIMIZATION_ENGINE_MODULE_v5.2.md) | Lambda + AppConfig |
| M3 | DR Dispatcher | v5.2 | [03_DR_DISPATCHER](./03_DR_DISPATCHER_MODULE_v5.2.md) | Lambda + EventBridge + MQTT |
| M4 | Market & Billing | v5.2 | [04_MARKET_BILLING](./04_MARKET_BILLING_MODULE_v5.2.md) | Lambda + DynamoDB |
| M5 | BFF | **v5.3** | [05_BFF](./05_BFF_MODULE_v5.3.md) | Lambda + API Gateway |
...
| 財務計算 | 凌晨批次計算 revenue_daily；日內 BFF 即時估算 |
| 外部數據 | 電價表、天氣快取入 DB，不依賴外部 API 即時請求 |
...
> 詳細 DDL 請見：`design/backend_architecture/10_DATABASE_SCHEMA_v5.4.md`
