...
| # | 文件名 | 路徑 | 說明 |
|---|--------|------|------|
| 00 | **MASTER_ARCHITECTURE** | `00_MASTER_ARCHITECTURE_v5.8.md` | 系統總控藍圖（本文件） |
...
| M2 | **OPTIMIZATION_ENGINE_MODULE** | [02_OPTIMIZATION_ENGINE_MODULE_v5.7.md](./02_OPTIMIZATION_ENGINE_MODULE_v5.7.md) | Optimization Engine — 4 種策略演算法、排程最佳化、雙目標框架、Schedule Generator Cron Job、動態電價優先策略 |
| M3 | **DR_DISPATCHER_MODULE** | [03_DR_DISPATCHER_MODULE_v5.6.md](./03_DR_DISPATCHER_MODULE_v5.6.md) | DR Dispatcher — 調度指令、SQS 逾時、狀態追蹤、Command Dispatcher Worker |
...
| M5 | **BFF_MODULE** | [05_BFF_MODULE_v5.5.md](./05_BFF_MODULE_v5.5.md) | Frontend BFF — 聚合 API、BFF 淨化行動、Cognito 授權 |
...
| Data | M2 | Optimization Engine | Schedule optimization, forecast, dual-objective optimization | SSM Parameter Store | Lambda + AppConfig |
...
| M2 | Optimization Engine | **v5.7** | [02_OPTIMIZATION_ENGINE](./02_OPTIMIZATION_ENGINE_MODULE_v5.7.md) | Lambda + AppConfig |
| M3 | DR Dispatcher | **v5.6** | [03_DR_DISPATCHER](./03_DR_DISPATCHER_MODULE_v5.6.md) | Lambda + EventBridge + MQTT |
| M4 | Market & Billing | **v5.8** | [04_MARKET_BILLING](./04_MARKET_BILLING_MODULE_v5.8.md) | Lambda + DynamoDB |
| M5 | BFF | **v5.5** | [05_BFF](./05_BFF_MODULE_v5.5.md) | Lambda + API Gateway |
...
```
           ...
           +--------+--------+         |        +v---------------------------+
                    |                   |        |  Module 3:                 |
                    |  ScheduleGenerated|        |  DR Dispatcher             |
                    |<-----------------+|        |  (Lambda + IoT Core        |
                    |  (->Device Shadow)|        |   MQTT publish +           |
                    |       EventBridge |        |   SQS Timeout Queue)       |
                    |                   |        |                            |
                    |                   +------->|                            |
                    |<---------------------------+
                    |
