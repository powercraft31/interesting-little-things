...
> **最後更新**: 2026-02-28
> **說明**: 電力市場數據接入（分時電價 TOU + CCEE 批發市場 PLD）、計費邏輯、收益計算、ROI、月度報告、PostgreSQL schema、Daily Billing Batch Job
...
M4 的每日結算 Job（每日 02:00 BRT 執行）由單軌改為雙軌計算：

#### B端計算：SOLFACIL 批發套利收益
```sql
-- 計算公式：∑(PLD_discharge - PLD_charge) × volume_kwh
-- 數據來源：trade_schedules（排程）+ pld_horario（電價）+ assets.submercado（地區）
INSERT INTO revenue_daily (org_id, date, vpp_arbitrage_profit_reais)
SELECT
  ts.org_id,
  CURRENT_DATE - INTERVAL '1 day' AS date,
  SUM(
    CASE ts.action
      WHEN 'discharge' THEN ts.expected_volume_kwh * p.pld_hora / 1000  -- MWh 換算
      WHEN 'charge'    THEN -ts.expected_volume_kwh * p.pld_hora / 1000
      ELSE 0
    END
...
JOIN pld_horario p ON
  ...
  p.dia = EXTRACT(DAY FROM ts.planned_time)::SMALLINT AND
  p.hora = EXTRACT(HOUR FROM ts.planned_time)::SMALLINT
WHERE ts.planned_time::DATE = CURRENT_DATE - INTERVAL '1 day'
GROUP BY ts.org_id;
```

#### C端計算：客戶省下的電費
```sql
-- 計算公式：∑ solar_direct_kwh × retail_buy_rate_kwh
-- 數據來源：telemetry_history（遙測）+ assets.retail_buy_rate_kwh
INSERT INTO revenue_daily (org_id, date, client_savings_reais, actual_self_consumption_pct)
SELECT
  a.org_id,
...
|------|----------------|------------|
| 電價來源 | pld_horario (R$/MWh) | assets.retail_buy_rate_kwh (R$/kWh) |
| 計算邏輯 | 套利差價 × 電量 | 太陽能直供 × 零售費率 |
| 目標 | 最大化套利利潤 | 最大化自發自用率 |
| 約束關係 | 受 target_self_consumption_pct 約束 | 由 vpp_strategies 設定門檻 |

---

## § v5.6 Daily Billing Batch Job Design

> **重要聲明：** 此 Batch Job 為 v5.6 簡化版，以 `trade_schedules` 推算收益。
> v5.8 將改為讀取真實遙測量 (`telemetry_history`) 進行精確計算。
...
| 處理範圍 | 昨天（`CURRENT_DATE - 1`）所有 `status = 'executed'` 的 trade_schedules |
...
| 資料來源 | 查詢條件 | 用途 |
|---------|---------|------|
| `trade_schedules` | `date(scheduled_at) = yesterday AND status = 'executed'` | 取得已執行的充放電紀錄 |
| `pld_horario` | `dia` 對應昨天的日期、`hora` 對應各筆 trade 的小時 | 取得對應時段的 PLD 電價 |
| `assets` | 透過 `asset_id` 關聯 | 取得 `retail_buy_rate_kwh`、`submercado` |

### Mock 算錢邏輯

> **⚠️ v5.6 簡化版：以 trade_schedules 的 power_kw × 1h 推算電量，非真實遙測量。**
> **v5.8 改為讀取 telemetry_history 的實際充放電量。**

#### B-side（SOLFACIL 套利）

```
arbitrage_profit = Σ (
  trade.power_kw × 1h × (pld_hora / 1000)   -- kW × 1h = kWh，÷1000 換算 MWh 單位
) WHERE trade.action = 'discharge'
```

計算邏輯：
- 只計算 `discharge`（放電）的套利收益
- `power_kw × 1h` = 該小時的放電量 (kWh)
- `pld_hora` 單位為 R$/MWh，除以 1000 換算為 R$/kWh
- 最終得到 SOLFACIL 的批發套利利潤 (R$)

#### C-side（客戶節省）

```
savings = Σ (
  trade.power_kw × 1h × asset.retail_buy_rate_kwh
) WHERE trade.action = 'discharge'
```

計算邏輯：
- 只計算 `discharge`（放電）時客戶省下的零售電費
- `power_kw × 1h` = 該小時的放電量 (kWh)
- `retail_buy_rate_kwh` 為客戶的零售電費費率 (R$/kWh)
- 最終得到客戶省下的電費金額 (R$)
...
INSERT INTO revenue_daily (
  ...
  arbitrage_profit_reais, savings_reais,
  revenue_reais, cost_reais, profit_reais
) VALUES (
  $1, $2, $3, $4, $5, $6, $7, $8
)
ON CONFLICT (asset_id, date) DO UPDATE SET
  arbitrage_profit_reais = EXCLUDED.arbitrage_profit_reais,
  savings_reais = EXCLUDED.savings_reais,
...
| 限制 | 說明 |
|------|------|
| 不讀取 telemetry_history | 不使用真實遙測數據（v5.8 才接入） |
| 不使用真實電力積分公式 | 以 `power_kw × 1h` 簡化推算，非 ∫P(t)dt |
| 數據來源 | 完全基於 trade_schedules + pld_horario 推算 |
| UPSERT 策略 | `ON CONFLICT (asset_id, date) DO UPDATE` — 重複執行不會產生重複數據 |
...
│   ├── daily-billing-batch.ts    # v5.6: Daily Billing Batch Job — Mock 推算
...
└── __tests__/
    ...
    └── daily-billing-batch.test.ts    # v5.6
```
...
| v5.6 | 2026-02-28 | Daily Billing Batch Job：凌晨讀取 executed trades × pld_horario，Mock 推算 revenue_daily |
...
| 方向 | 模組 | 說明 |
|------|------|------|
| **依賴** | M2 (Optimization Engine) | 消費 `ScheduleGenerated` 記錄預期收益；v5.6: 讀取 trade_schedules status=executed |
