  ...
  "assets": [
    {
      ...
      "config": {
        ...
        "min_soc": 20,
        "max_soc": 90,
        "target_self_consumption_pct": 70,
        "max_charge_rate": 3.3,
...
### Subsection A — get-assets.ts Config Fix

#### 問題陳述

當前 `get-assets.ts` 使用 `CONFIG_DEFAULTS` 靜態物件提供 asset config 欄位：

```typescript
// ❌ HARDCODED since v5.3, TODO since v5.6
...
所有 asset 都回傳相同的 config，忽略 `vpp_strategies` 表中各 operation_mode 的真實配置。

#### 修正

**移除** `CONFIG_DEFAULTS` 靜態物件。

**升級 SQL 查詢**，加入 LEFT JOIN vpp_strategies：

```sql
-- v5.9: 從 vpp_strategies 讀取真實配置
SELECT
  a.asset_id,
  a.org_id,
  a.region,
...
LEFT JOIN vpp_strategies vs ON vs.operation_mode = a.operation_mode
WHERE a.org_id = $1
ORDER BY a.asset_id;
```

**Response config 欄位映射：**

```typescript
config: {
  target_mode: row.operation_mode,
  min_soc: row.min_soc,                           // from vpp_strategies (nullable)
  max_soc: row.max_soc,                           // from vpp_strategies (nullable)
  target_self_consumption_pct: row.target_self_consumption_pct,  // from vpp_strategies (nullable)
  charge_window_start: row.charge_window_start,    // from vpp_strategies (nullable)
  charge_window_end: row.charge_window_end,        // from vpp_strategies (nullable)
  discharge_window_start: row.discharge_window_start // from vpp_strategies (nullable)
}
```

**Fallback 策略：** 若 `vpp_strategies` 無對應 `operation_mode` 的記錄，回傳 `null` config 欄位。
Frontend 需 gracefully handle null values（顯示 "—" 或 "未配置"）。

---

### Subsection B — get-dashboard.ts KPI Fix

#### 問題陳述

當前 `get-dashboard.ts` 包含 **7 個 hardcoded 值**：

```typescript
// ❌ HARDCODED KPIs
const dashboard = {
  dispatchSuccessCount: 156,         // hardcoded
  dispatchTotalCount: 160,           // hardcoded
  vppDispatchAccuracy: 97.5,         // hardcoded
  drResponseLatency: 1.8,            // hardcoded
  gatewayUptime: 99.9,               // hardcoded
  monthlyRevenueReais: 0,            // hardcoded
  dispatchSuccessRate: '156/160',    // hardcoded
};
```
...
```sql
-- v5.9: 從 revenue_daily 計算當月累計收益
SELECT COALESCE(SUM(profit_reais), 0) AS monthly_revenue_reais
FROM revenue_daily
...
```sql
-- v5.9: 從 dispatch_commands 計算今日成功數
SELECT COUNT(*) AS dispatch_success_count
...
```sql
-- v5.9: 從 dispatch_commands 計算今日總數
SELECT COUNT(*) AS dispatch_total_count
FROM dispatch_commands
WHERE dispatched_at >= CURRENT_DATE;
```
...
```typescript
// v5.9: 計算字串格式 "{success}/{total}"
const dispatchSuccessRate = `${dispatchSuccessCount}/${dispatchTotalCount}`;
```

#### 保留的靜態 KPIs（System-Level，明確標註）

以下 3 個 KPI 為系統層級指標，目前無 Prometheus / CloudWatch 數據源，
保留為靜態展示值，並加上明確註解：

```typescript
// DEMO DISPLAY VALUES — System Level, no Prometheus source yet
// These will be replaced when observability pipeline (Prometheus/CloudWatch) is connected
const systemKpis = {
  vppDispatchAccuracy: 97.5,  // DEMO DISPLAY VALUE (System Level — no Prometheus source)
  drResponseLatency: 1.8,     // DEMO DISPLAY VALUE (System Level)
  gatewayUptime: 99.9,        // DEMO DISPLAY VALUE (System Level)
};
```

#### 完整 Dashboard Response（v5.9）
...
const dashboard = {
  ...
  avgSoc,
  totalPowerKw,
  dailyRevenueReais,        // from v5.5 (revenue_daily)
  monthlyRevenueReais,      // ✅ v5.9: from DB (was hardcoded 0)
  dispatchSuccessCount,     // ✅ v5.9: from DB (was hardcoded 156)
  dispatchTotalCount,       // ✅ v5.9: from DB (was hardcoded 160)
  dispatchSuccessRate,      // ✅ v5.9: computed "{success}/{total}" (was hardcoded "156/160")
  vppDispatchAccuracy: 97.5,  // DEMO DISPLAY VALUE (System Level — no Prometheus source)
  drResponseLatency: 1.8,     // DEMO DISPLAY VALUE (System Level)
  gatewayUptime: 99.9,        // DEMO DISPLAY VALUE (System Level)
  vppArbitrageProfit,       // from v5.5 (revenue_daily)
  clientSavings,            // from v5.5 (revenue_daily)
  selfConsumptionPct,       // from v5.5 (algorithm_metrics)
};
```
...
src/bff/
├── handlers/
│   ├── get-dashboard.ts          # GET /dashboard — aggregated KPI data (v5.5: dual-layer; v5.9: DB-sourced KPIs)
│   ├── get-assets.ts             # GET /assets — list all assets (v5.9: vpp_strategies JOIN, no CONFIG_DEFAULTS)
...
└── __tests__/
    ├── get-dashboard.test.ts     # v5.9: DB-sourced KPI tests
    ├── get-assets.test.ts        # v5.9: vpp_strategies JOIN tests
...
| **依賴** | M3 (DR Dispatcher) | 發佈 DRCommandIssued、查詢調度結果；**v5.9: 讀取 dispatch_commands 計算 KPI** |
| **依賴** | M4 (Market & Billing) | PostgreSQL 查詢收益/電價；**v5.9: 讀取 revenue_daily 計算 monthlyRevenueReais** |
| **依賴** | M6 (Identity) | Cognito Authorizer、JWT 驗證 |
| **依賴** | M8 (Admin Control) | AppConfig feature-flags 讀取；**v5.9: LEFT JOIN vpp_strategies 讀取 asset config** |
