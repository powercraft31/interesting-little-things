...
| # | 文件名 | 路徑 | 說明 |
|---|--------|------|------|
| 00 | **MASTER_ARCHITECTURE** | `00_MASTER_ARCHITECTURE_v5.7.md` | 系統總控藍圖（本文件） |
| M1 | **IOT_HUB_MODULE** | [01_IOT_HUB_MODULE_v5.3.md](./01_IOT_HUB_MODULE_v5.3.md) | IoT Hub — MQTT 接入、動態解析器、Asset Shadow |
...
| M4 | **MARKET_BILLING_MODULE** | [04_MARKET_BILLING_MODULE_v5.6.md](./04_MARKET_BILLING_MODULE_v5.6.md) | Market & Billing — 電價計費、雙軌收益計算、Daily Billing Batch Job |
...
| 10 | **DATABASE_SCHEMA** | [10_DATABASE_SCHEMA_v5.7.md](./10_DATABASE_SCHEMA_v5.7.md) | PostgreSQL 完整 DDL — 18 張表、ER 圖、Migration 管理 |
...
> 3. **M4 Daily Billing Batch Job** — 凌晨讀取 executed trades × `pld_horario`，更新 `revenue_daily`（node-cron，每日 00:05 執行）
>
...
| Data | M4 | Market & Billing | Tarifa Branca rules, PLD arbitrage, dual-track billing, invoicing | RDS PostgreSQL | Lambda + DynamoDB |
...
| Shared | Database Schema | **v5.7** | [10_DATABASE_SCHEMA](./10_DATABASE_SCHEMA_v5.7.md) | PostgreSQL — 18 張表 |
| M1 | IoT Hub | **v5.3** | [01_IOT_HUB](./01_IOT_HUB_MODULE_v5.3.md) | Lambda + IoT Core + DynamoDB |
...
| M4 | Market & Billing | **v5.6** | [04_MARKET_BILLING](./04_MARKET_BILLING_MODULE_v5.6.md) | Lambda + DynamoDB |
...
| 財務計算 | 凌晨批次計算 revenue_daily（雙軌：B端套利 + C端省電）；日內 BFF 即時估算 |
| 外部數據 | 電價表、天氣快取、CCEE PLD 電價入 DB，不依賴外部 API 即時請求；v5.7 起由 M7 Inbound Webhook 動態更新 |
| 模組邊界 | 嚴禁跨模組直接 SQL JOIN，必須透過 Service 層 |
...
| **trade_schedules** | M2 Optimization | 排程表 | **v5.5** — M2 最佳化排程輸出 |
...
> 詳細 DDL 請見：`design/backend_architecture/10_DATABASE_SCHEMA_v5.7.md`
