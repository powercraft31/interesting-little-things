...
> **模組版本**: v5.5
> **變更**: v5.5 — TradeRecord 單位標示、DashboardKPI 雙層結構、雙層經濟模型 API 契約說明
...
export interface TradeRecord {
  ...
  targetPldPrice: number | null; // R$/MWh（批發市場電價，非零售）
  createdAt: string;
}
```

> **[v5.5 變更說明]** `targetPldPrice` 單位為 R$/MWh（CCEE 批發市場電價），
> 不得與 C端零售電價（R$/kWh）混用。數據來源：`trade_schedules` 表。

### 2.9 DashboardKPI (v5.5 — 雙層結構)

```typescript
// src/shared/types/dashboard-kpi.ts

export interface DashboardKPI {
  // B端：SOLFACIL 批發套利收益
  vppArbitrageProfit: number;     // R$/MWh 套利收益（今日）
  vppArbitrageProfitMonthly: number;  // 本月累計

  // C端：客戶節省電費
  clientBillSavings: number;      // R$（今日）
  clientBillSavingsMonthly: number;   // 本月累計

  // C端健康度指標
  selfConsumptionPct: number;     // % 自發自用率

  // 系統運行指標
  totalAssetsOnline: number;
  totalAssetsOffline: number;
  totalEnergyDispatched: number;  // kWh（今日）
}
```

> **[v5.5 變更說明]** 取代 v5.3 的 `DashboardMetrics`，拆分為 B端/C端雙層結構。
> `dailyRevenueReais` / `monthlyRevenueReais` 已拆分為
> `vppArbitrageProfit`（B端）與 `clientBillSavings`（C端），不再合併為單一數字。

### 2.10 DashboardMetrics (v5.3 — 保留供向下相容)

```typescript
// src/shared/types/dashboard.ts
// ⚠️ DEPRECATED in v5.5 — 請使用 DashboardKPI (§2.9)
...
**嚴禁：**
- 將 B端和 C端收益加總為單一 `revenue` 數字
- 在 C端介面顯示 R$/MWh 單位
- 在 B端報表使用 R$/kWh 單位

---

## 4. AppConfig Parser Rules — `vpp-m1-parser-rules` Profile
...
│   │       ├── telemetry.ts
│   │       ├── events.ts
│   │       ├── trade.ts                 # v5.5 TradeRecord
│   │       ├── dashboard-kpi.ts         # v5.5 DashboardKPI
│   │       └── auth.ts
│   ├── iot-hub/                          # Module 1
...
| Self-consumption % (v5.5) | M2 | < target_self_consumption_pct |
...
## 9. Database Connection Pool（v5.4 新增）
...
| **依賴** | [00_MASTER_ARCHITECTURE](./00_MASTER_ARCHITECTURE_v5.5.md)（上層文件） |
