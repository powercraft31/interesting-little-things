# 10. Database Schema — v5.8
> **版本**: v5.8 | **建立日期**: 2026-03-02 | **負責人**: Shared Infrastructure
>
> **變更說明**: v5.8 Closed-loop Billing via Data Contract — 新增 `asset_hourly_metrics` 表（Shared Contract），
> 由 M1 Aggregator Job 每小時寫入，作為 M4 財務結算的唯一數據來源。
> M4 切換為真實度數結算，`trade_schedules.expected_kwh` 降級為排程參考。
> 表格總數：19。
...
> **v5.5 新增 3 張表說明**：
> - `pld_horario`：CCEE 批發市場數據來源（dadosabertos.ccee.org.br），每日更新，4 個子市場 x 24 小時
> - `trade_schedules`：M2 最佳化引擎的排程輸出，供 M5 BFF 讀取展示
> - `algorithm_metrics`：僅商業健康度指標（self_consumption_pct），不含技術精度指標（alpha/MAPE）

> **v5.8 新增 1 張表說明**：
> - `asset_hourly_metrics`：Shared Contract 表，由 M1 Hourly Aggregator Job 每小時寫入。
>   M4 Daily Billing Batch Job 讀取此表進行真實度數結算。嚴禁直接讀取 `telemetry_history` 替代。
...
-- v5.4 HEMS 單戶場景：capacity_kwh 取代舊版 unidades（聚合器欄位已移除）
-- v5.5 新增：submercado / retail_buy_rate_kwh / retail_sell_rate_kwh
CREATE TABLE assets (
...
-- v5.8 說明：自 v5.8 起，此表為 M1 內部原始時序數據，不對外暴露。
-- 外部模組應讀取 asset_hourly_metrics。
CREATE TABLE telemetry_history (
  id             BIGSERIAL,
  asset_id       VARCHAR(50)  NOT NULL,
  -- 架構決策：此欄位刻意不建立 REFERENCES assets(asset_id) 的外鍵約束。
  -- 原因：telemetry_history 是高併發寫入的時序表（每 5 分鐘 x 全量設備）。
...
-- ============================================================
-- 自 v5.8 起，此表由 M1 Aggregator Job 每小時寫入，作為 M4 財務結算的唯一數據契約 (Data Contract)。
-- 嚴禁其他模組直接讀取 telemetry_history 替代此表。

CREATE TABLE asset_hourly_metrics (
  id              BIGSERIAL PRIMARY KEY,
  asset_id        UUID NOT NULL REFERENCES assets(id),
  hour_timestamp  TIMESTAMPTZ NOT NULL,  -- truncated to hour, e.g. 2026-03-01 14:00:00+08
  total_charge_kwh    NUMERIC(10,4) NOT NULL DEFAULT 0,
  total_discharge_kwh NUMERIC(10,4) NOT NULL DEFAULT 0,
  data_points_count   INT NOT NULL DEFAULT 0,  -- number of telemetry records aggregated
  created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
-- M4 query pattern: WHERE hour_timestamp >= yesterday_start AND hour_timestamp < today_start
CREATE INDEX idx_asset_hourly_hour ON asset_hourly_metrics (hour_timestamp DESC);

COMMENT ON TABLE asset_hourly_metrics IS
  '自 v5.8 起，此表由 M1 Aggregator Job 每小時寫入，作為 M4 財務結算的唯一數據契約 (Data Contract)。嚴禁其他模組直接讀取 telemetry_history 替代此表。';
...
-- v5.7 說明：此表自 v5.7 起由 M7 Inbound Webhook (POST /webhooks/weather) 動態接收天氣資料。
-- 欄位：location (地區代碼), recorded_at (預報時間), temperature, irradiance, cloud_cover, source
-- v5.7 Mock Publisher 每 30 分鐘推送一次；v6.0 替換為真實氣象 API。
-- M2 未來可讀取 irradiance 輔助太陽能發電量預測。

-- 每日財務結算（凌晨批次計算後寫入）
-- v5.5 新增：vpp_arbitrage_profit_reais / client_savings_reais / actual_self_consumption_pct
...
COMMENT ON COLUMN revenue_daily.vpp_arbitrage_profit_reais IS 'B端：∑(PLD_discharge - PLD_charge) x kWh，進 SOLFACIL 口袋';
COMMENT ON COLUMN revenue_daily.client_savings_reais IS 'C端：∑ solar_direct_kwh x retail_buy_rate，客戶電費節省';
...
-- v5.7 說明：自 v5.7 起，此表由 M7 Inbound Webhook (POST /webhooks/ccee-pld) 動態更新。
-- 外部 CCEE 推送（或 Mock Publisher）的電價透過 ON CONFLICT DO UPDATE 覆寫。
-- M2 Schedule Generator 優先讀取最新電價，不再依賴 v5.6 的全歷史平均值。
...
-- v5.8 說明：此表的 expected_kwh 欄位自 v5.8 起僅供排程參考，不再用於最終財務結算。
-- 真實度數來源：asset_hourly_metrics。
...
-- v5.5 新增：target_self_consumption_pct
...
-- 注意：SOLFACIL_ADMIN 超管帳號需使用 BYPASSRLS 屬性的 DB role，
...
```sql
-- ============================================================
-- v5.8 Migration: 新增 asset_hourly_metrics (Shared Contract)
-- ============================================================
