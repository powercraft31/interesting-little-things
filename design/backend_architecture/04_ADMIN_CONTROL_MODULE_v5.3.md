...
### Data Dictionary Seed Records (v5.3)

以下為 v5.3 新增的 Global Data Dictionary 初始記錄，部署時須寫入 `vpp-data-dictionary` DynamoDB 表：

```json
[
  {
    "fieldKey": "capacity_kwh",
    "label": "系統電池容量",
    "type": "static",
    "dataType": "number",
    "unit": "kWh",
    "description": "電池系統總裝機容量，由安裝商在設備 commissioning 時設定。取代原 unidades 聚合器欄位，對齊 HEMS 單戶家庭場景。v5.3 新增。",
    "required": true,
    "guardRail": false
  },
  {
    ...
    "dataType": "string",
    "unit": null,
    "description": "資產當前運行狀態。允許值：'operando'（運行中）、'carregando'（充電中）、'offline'（離線）。v5.3 確認命名規範（取代舊版 status 字串欄位）。",
    "required": true,
    "guardRail": true
  }
]
...
> **[v5.3 字典移除聲明]** `unidades` 欄位已於 v5.3 正式從全局字典移除。
> 原因：該欄位源自 VPP 聚合器設計（代表管理的用戶數量），
> 與 HEMS 單戶家庭場景相矛盾。所有模組應停止讀取此欄位。
> 若有歷史數據仍包含此欄位，BFF 層應在 response mapping 中靜默過濾。
