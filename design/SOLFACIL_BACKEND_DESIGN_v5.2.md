...
v5.2 introduces **Runtime Schema Evolution** — the ability to add new telemetry fields to the entire VPP pipeline without code changes, deployments, or PR reviews. Two key changes make this possible:

1. **M1 Flexible Schema (Business Trilogy Data Model):** The flat, hardcoded `StandardTelemetry` interface is replaced with a dynamic envelope containing three domain buckets — `metering`, `status`, and `config`. Field mappings are externalized to an AWS AppConfig profile (`vpp-m1-parser-rules`), turning M1 into a pure **translation executor**.

2. **M8 Global Data Dictionary:** M8 gains a new `vpp-data-dictionary` DynamoDB table that serves as the canonical registry of every telemetry field in the system. When an operator adds a field via the Control Plane UI, M8 performs an atomic dual-write (AppConfig + DynamoDB) and publishes a `SchemaEvolved` EventBridge event — enabling all downstream modules to auto-discover new fields.

**Business Impact:** New field onboarding drops from **2–5 business days** (developer + code change + PR + deploy) to **~90 seconds** (operator clicks Deploy in the UI).

| Aspect | v5.1 | v5.2 |
|--------|------|------|
| **M1 StandardTelemetry** | Flat interface with hardcoded fields (`battery_soc`, `grid_power_kw`, etc.) | Dynamic envelope with `metering` / `status` / `config` domain buckets |
| **M1 Field Mapping** | Code-level adapters (`HuaweiAdapter`, `SungrowAdapter`) | AppConfig `vpp-m1-parser-rules` profile — JSON-driven, hot-reloadable |
| **M8 Field Registry** | No centralized field catalog | Global Data Dictionary (`vpp-data-dictionary` DynamoDB table) |
| **New Field Process** | Developer writes adapter code → PR → review → deploy (2–5 days) | Operator uses Parser Editor UI → Deploy → 90 seconds |
| **Cross-Module Discovery** | Manual — each module must be updated to know about new fields | Automatic — `SchemaEvolved` EventBridge event notifies all modules |
...
| **v5.1** | 2026-02-21 | **Cloud-Native Upgrade.** §0.2 replaced ElastiCache Redis + EventBridge broadcast (anti-pattern) with AWS AppConfig + Lambda Extension (sidecar pattern). Eliminates Redis ops cost, TTL management code, and config-refresh Lambdas. Adds Canary Deployment + Auto-Rollback safety for M8 config publishing. Added AppConfig JSON Schema Validators (Shift-Left Validation) for zero-blast-radius config protection. Added §0.3 Observability & Distributed Tracing: AWS X-Ray active tracing, vpp-{UUID} trace_id propagation across M1→M2→M3, structured JSON logging standard, and CloudWatch Alarm integration with AppConfig auto-rollback. |
| **v5.2** | 2026-02-25 | **Runtime Schema Evolution & Global Data Dictionary.** (1) M1 rewritten as pure translation executor — all field mapping logic externalized to AppConfig `vpp-m1-parser-rules` profile; (2) Business Trilogy Data Model (metering/status/config); (3) M8 expanded with Global Data Dictionary service and `SchemaEvolved` EventBridge cascade; (4) Control Plane UI M1 Parser Editor (three-panel design); (5) New field onboarding: 2–5 business days → 90 seconds. |
...
0. [Architectural Law: Control Plane vs. Data Plane](#0-architectural-law-control-plane-vs-data-plane-全局法則)
   ...
   - 0.3 [Observability & Distributed Tracing — AWS X-Ray + Structured Logging](#03-observability--distributed-tracing--aws-x-ray--structured-logging)
...
21. [Control Plane UI — M1 Parser Editor](#21-control-plane-ui--m1-parser-editor-解析器視覺化編輯器)
22. [Runtime Schema Evolution — Magic Workflow](#22-runtime-schema-evolution--magic-workflow-零代碼跨模塊聯動)
23. [Migration & Upgrade Path (v5.1 → v5.2)](#23-migration--upgrade-path-v51--v52)
...
- **v5.2 addition:** Owns the **Global Data Dictionary** — canonical registry of every telemetry field
- **v5.2 addition:** Publishes `SchemaEvolved` events via EventBridge when field definitions change
...
|--------|-------------------|----------|-------------|-----------|-------------------|
| **M1** IoT Hub | Device parser rules — AppConfig-driven `vpp-m1-parser-rules` profile (v5.2: field mappings externalized as JSON rules, hot-reloadable via Lambda Extension) | `device_parser_rules` | Lambda cold start + `ConfigUpdated{module:'M1'}` | 5 min | `parser_rules:{org_id}` |
...
- vpp-m1-parser-rules（M1, v5.2）：JSON-driven field mapping rules for the Business Trilogy Data Model (metering/status/config)
...
**AppConfig JSON Schema Validators（左移驗證 Shift-Left Validation）**

每個 Configuration Profile 在建立時必須綁定對應的 JSON Schema 驗證器。這實現了「Shift-Left Validation」——配置錯誤在按下發佈的「第 0 秒」被攔截，連 Canary 金絲雀階段都進不去。

驗證觸發時機：M8 Admin Lambda 呼叫 `AppConfig:StartDeployment` → AppConfig 自動對 Content 執行 JSON Schema 驗證 → 驗證失敗 → 立即拋出 `BadRequestException`，Deployment 直接 Rejected，PostgreSQL 回寫失敗狀態。

核心 Schema 範例（vpp-strategies profile）：
...
防爆效果：若 M8 操作員誤輸入 `min_soc: 200`，AppConfig 在 Schema 驗證階段立即拒絕（`minimum: 10, maximum: 50`），錯誤配置不會進入 Canary 階段，實現真正的「零爆炸半徑（Zero Blast Radius）」。

各 Profile 對應的核心約束：
- parser-rules：field_mapping 必須是 object，unit_conversions 的 factor 必須是正數（minimum: 0.0001）
- vpp-strategies：min_soc < max_soc（使用 if/then 依賴校驗），emergency_soc < min_soc
- dispatch-policies：max_retry_count 最大值 5，timeout_minutes 範圍 5-60
- billing-rules：operating_cost_per_kwh 必須是正數（minimum: 0.001）
- feature-flags：is_enabled 必須是 boolean，不允許 string "true"
- api-quotas：calls_per_minute 最大值 10000，burst_limit 不得超過 calls_per_minute × 2
...
### 0.3 Observability & Distributed Tracing — AWS X-Ray + Structured Logging

**設計原則：Every Request Has a Story**

在 VPP 系統中，一筆電池充電指令的生命週期橫跨 M1→M2→M3，涉及多個非同步 EventBridge 事件和 Lambda 調用。如果沒有分散式追蹤，當客戶問「為什麼我的電池昨晚 3 點沒有按策略充電？」，工程師只能在 CloudWatch Logs 的茫茫記錄中大海撈針。

X-Ray 讓每一次跨模塊的非同步調用，都能在 AWS Console 的視覺化瀑布圖（Service Map）中精準重現。

---

**全局 X-Ray 啟用規範**

所有 Lambda 函數在 CDK 定義時必須啟用 X-Ray active tracing：

```typescript
// 所有 Lambda 定義的強制規範（CDK）
...
所有 API Gateway 路由必須啟用 X-Ray tracing：
...
**trace_id 生命週期與跨模塊傳播規範**

trace_id 是本系統分散式追蹤的靈魂。以下是它的完整傳播路徑：

**Step 1：M1 生成 trace_id（所有非同步鏈的起點）**

M1 的 `ingest-telemetry.ts` Lambda 在接收到 MQTT 消息後，若判斷需要觸發策略計算，必須生成唯一的 trace_id 並附加到所有下游調用：

```typescript
// M1 ingest-telemetry.ts（偽代碼，v5.x 實作目標）
import { randomUUID } from 'crypto';

const traceId = `vpp-${randomUUID()}`; // 格式：vpp-{UUID}

// 寫入 Timestream 同時記錄 trace_id
await writeToTimestream({ ...telemetry, traceId });

// 發佈 EventBridge 事件時必須包含 trace_id
...
**Step 2：M2 接收並繼承 trace_id**

M2 的 `run-optimization.ts` Lambda 從 EventBridge event.detail 中提取 traceId，並在生成 DR 指令時繼續傳遞：

```typescript
// M2 run-optimization.ts（偽代碼）
...
// 發佈 DRCommandIssued 時繼續攜帶 traceId
await eventBridge.putEvents({
  Entries: [{
...
M3 的 `dispatch-command.ts` Lambda 接收 traceId，寫入 DynamoDB 命令記錄，並在廣播 MQTT 指令時記錄最終節點：

```typescript
// M3 dispatch-command.ts（偽代碼）
const { commandId, deviceId, traceId } = event.detail;

// 寫入 DynamoDB 時帶入 traceId，供後續審計
await dynamoDB.put({
...
所有 Lambda 的 console.log 必須輸出 JSON 格式，包含以下必填欄位：
...
```json
{
  ...
  "action": "動作描述（如 telemetry_ingested, optimization_result, command_dispatched）",
...
CloudWatch Logs Insights 查詢範例（追蹤單一 trace_id 的完整鏈路）：
...
**X-Ray Service Map 視覺化效果**

啟用後，AWS Console → X-Ray → Service Map 中可看到：
...
每個節點顯示：P50/P95/P99 延遲、Error Rate、Throughput。當任何節點出現異常，工程師在 30 秒內定位到問題模塊，無需翻閱數千行 log。

---

**可觀測性三支柱整合（Metrics + Logs + Traces）**
...
### 4.1 StandardTelemetry v5.2 — Dynamic Schema Envelope (動態 Schema 信封)

v5.2 replaces the flat, hardcoded `StandardTelemetry` interface with a flexible envelope built around the **Business Trilogy Data Model** — three domain buckets that categorize every telemetry field:
...
export interface StandardTelemetry {
  ...
  // ... brittle, every new field = code change
}
...
export interface StandardTelemetry {
  ...
  orgId: string;
  timestamp: string;
  traceId: string;            // vpp-{UUID} propagated from M1
  metering: Record<string, number>;    // e.g. { "metering.grid_power_kw": 3.2 }
  status:   Record<string, string | number | boolean>;  // e.g. { "status.battery_soc": 85 }
  config:   Record<string, string | number | boolean>;  // e.g. { "config.charge_limit": 90 }
}
```

This design means adding a new telemetry field (e.g., `status.chiller_temp`) requires **zero code changes** — only a new parser rule in AppConfig.

### 4.2 AppConfig Parser Rules — `vpp-m1-parser-rules` Profile (解析規則配置)

All field mapping logic is externalized to an AWS AppConfig Configuration Profile named `vpp-m1-parser-rules`. M1 reads these rules via the Lambda Extension sidecar and applies them at runtime.
...
M1 implements a three-tier fallback strategy to ensure telemetry ingestion never stops, even if AppConfig is unavailable:
...
│   └── translation-executor.ts   # v5.2: translateGatewayPayload() — AppConfig rule engine
├── contracts/
│   └── standard-telemetry.ts     # v5.2: Dynamic schema envelope (Business Trilogy)
└── __tests__/
    ├── ingest-telemetry.test.ts
    ├── timestream-writer.test.ts
    └── translation-executor.test.ts  # v5.2: rule application tests
```
...
**Global Data Dictionary（v5.2 新增）：** M8 now owns the canonical registry of every telemetry field in the system. The `vpp-data-dictionary` DynamoDB table serves as the Global Data Dictionary — enabling cross-module field discovery, self-documenting schemas, and zero-code field onboarding via the Control Plane UI.

---

### 20.2 M8 Core Data Tables (PostgreSQL, 與 M4 同一 VPC)
...
### 20.3 Global Data Dictionary Service — New in v5.2 (新增資料字典服務)

The Global Data Dictionary is a new M8 service that provides a canonical registry of every telemetry field in the VPP system. It enables cross-module field discovery, self-documenting schemas, and zero-code field onboarding.

#### DataDictionaryEntry Type
...
Attributes:
  ...
  createdBy     (String)     — userId who created this entry
  createdAt     (String)     — ISO 8601 timestamp

GSI: domain-index
  PK: domain
  SK: fieldId
  Purpose: "List all metering fields across all orgs" (admin query)
```

#### Atomic Dual-Write: AppConfig + DynamoDB

When an operator adds a new field via the Control Plane UI (§21), M8 performs an **atomic dual-write** to ensure consistency between the parser rules (consumed by M1) and the data dictionary (consumed by all modules):
...
async function publishNewField(entry: DataDictionaryEntry, parserRule: ParserRule): Promise<void> {
  ...
  // 3. Publish SchemaEvolved event (all modules auto-discover new field)
  await eventBridge.putEvents({
    Entries: [{
      Source: 'vpp.m8.admin',
...
#### SchemaEvolved EventBridge Event

When field definitions change (add, update, or remove), M8 publishes a `SchemaEvolved` event:
...
| Module | Reaction to `SchemaEvolved` |
|--------|-----------------------------|
| M1 (IoT Hub) | No action needed — AppConfig Extension auto-reloads parser rules |
| M2 (Algorithm Engine) | Invalidates field-selector cache; new field appears in strategy editor |
| M3 (DR Dispatcher) | No action needed (does not consume telemetry fields directly) |
| M4 (Market & Billing) | Updates field selector dropdown for billing rule configuration |
| M5 (Frontend BFF) | Invalidates dashboard field cache; new field available in UI widgets |
| M7 (Open API) | Updates API schema documentation; new field available in webhook payloads |

#### Cross-Module Field Discovery Flow

Any module can query the Data Dictionary to discover available fields:

```typescript
// Example: M2 querying available metering fields for strategy optimization
const fields = await dynamoDB.query({
...
| Method | Endpoint | Description | Min Role |
|--------|----------|-------------|----------|
| `GET` | `/admin/data-dictionary` | List all fields for the org | `ORG_MANAGER` |
...
### 20.5 CDK Stack (v5.0+ 實作)

> **Note:** Infrastructure-as-Code details for v5.0+ implementation.
...
| `AdminDataDictionaryLambda` | v5.2: CRUD operations for `vpp-data-dictionary` + atomic dual-write |
...
## 21. Control Plane UI — M1 Parser Editor (解析器視覺化編輯器)

> **Status:** NEW in v5.2
> **Purpose:** Visual interface for operators to map raw MQTT telemetry fields to the Business Trilogy Data Model without writing code.

### 21.1 Three-Panel Layout

The M1 Parser Editor uses a three-panel drag-to-bind design:
...
|-------|---------|-------------|
| **Left: Live MQTT Tree View** | Real-time gateway payload rendered as an expandable JSON tree with full field paths | Click to select a source path |
| **Center: Drag-to-Bind Arrows** | Visual connection lines between source (left) and target (right) fields | Drag from left node to right slot |
| **Right: Business Trilogy Tabs** | Three tabs (Metering / Status / Config) each with mapped fields and "+ Add Field" button | Add new fields, configure transforms |

### 21.2 Operator Workflow — Adding `status.chiller_temp`

**Scenario:** A field engineer needs to add a new `status.chiller_temp` field from a gateway that reports chiller temperature.

**Step-by-step:**

1. **Gateway sends MQTT payload** containing `cooling.chiller.temp_celsius: 42.3`

2. **Operator sees it in left panel tree** — the Live MQTT Tree View displays the real payload from the gateway in real-time, showing the full JSON structure with `cooling.chiller.temp_celsius` highlighted as an unmapped field

3. **Clicks "+ Add Field" in Status tab** — opens a field creation form in the right panel

4. **Enters field metadata:**
   - Field Name: `status.chiller_temp`
   - Display Name: "Chiller Temperature"
   - Type: `number`
   - Unit: `°C`

5. **Drags from `cooling.chiller.temp_celsius` to the new field slot** — creates a visual binding arrow in the center panel, auto-populating `sourcePath: "cooling.chiller.temp_celsius"`

6. **System auto-suggests transform** — since the source value (42.3) is already a reasonable number in the expected unit, the system suggests `identity`. For a value in milliwatts that should be kW, it would suggest `divide:1000000`

7. **Operator clicks Deploy** — triggers the M8 atomic dual-write workflow

8. **M8 executes:**
   - Validates entry against JSON Schema
   - Writes `DataDictionaryEntry` to `vpp-data-dictionary` DynamoDB table
   - Creates new AppConfig version for `vpp-m1-parser-rules`
   - Publishes `SchemaEvolved` event to EventBridge
   - All downstream modules auto-discover `status.chiller_temp`

### 21.3 Live Preview

The Live Preview panel in the bottom-right shows a real-time sample of the `StandardTelemetry` output that would be produced with the current rule configuration:
...
```json
{
  ...
  "orgId": "ORG_ENERGIA_001",
  "timestamp": "2026-02-25T14:30:00.000Z",
  "traceId": "vpp-a1b2c3d4-...",
  "metering": {
    "metering.grid_power_kw": 3.2,
    "metering.voltage": 220.5
  },
  "status": {
    "status.battery_soc": 85,
    "status.chiller_temp": 42.3
  },
  "config": {}
}
```

The newly added `status.chiller_temp` field appears immediately in the preview, giving the operator confidence before clicking Deploy.

---

## 22. Runtime Schema Evolution — Magic Workflow (零代碼跨模塊聯動)

> **Status:** NEW in v5.2
> **Purpose:** Demonstrates the end-to-end zero-code field onboarding flow, from operator action to system-wide field availability.

### 22.1 Scenario

A field engineer adds `status.chiller_temp` via the M1 Parser Editor (§21). No code changes, no deployments, no PR reviews.

### 22.2 Timeline

| Time | Event |
|------|-------|
| T+0s | Engineer clicks **Deploy** in Control Plane UI |
| T+1s | M8 validates entry against JSON Schema |
| T+3s | M8 writes to DynamoDB `vpp-data-dictionary` |
| T+5s | M8 creates new AppConfig version for `vpp-m1-parser-rules` |
| T+6s | M8 publishes `SchemaEvolved` to EventBridge |
| T+15s | EventBridge delivers to M2, M3, M4, M5, M7 Lambda subscribers |
| T+15s | M2 Algorithm Engine receives event, invalidates its field-selector cache |
| T+90s | All Lambda instances hot-reload AppConfig via Extension sidecar |
| T+90s | M1 now translates `cooling.chiller.temp_celsius` → `status.chiller_temp` |
| T+91s | M4 billing module field selector dropdown auto-populates `status.chiller_temp` |
| T+91s | M2 optimization strategy editor shows `status.chiller_temp` as available input |

**Zero code changes. Zero deployments. Zero PR reviews.**

### 22.3 Business Impact — v5.1 vs v5.2

**v5.1 Process (2–5 business days):**

| Step | Action | Who | Duration |
|------|--------|-----|----------|
| 1 | Discover new field in MQTT payload | Field Engineer | ~1 hour |
| 2 | File Jira ticket requesting new field support | Field Engineer | ~30 min |
| 3 | Developer writes adapter code (or modifies existing adapter) | Backend Developer | ~4 hours |
| 4 | Developer writes unit tests for new field mapping | Backend Developer | ~2 hours |
| 5 | PR review + approval + merge + CI/CD deploy | Team Lead | ~1-3 business days |

**v5.2 Process (~90 seconds):**

| Step | Action | Who | Duration |
|------|--------|-----|----------|
| 1 | Open M1 Parser Editor in Control Plane UI | Operator | ~10 seconds |
| 2 | See field in Live MQTT Tree, drag to target slot | Operator | ~30 seconds |
| 3 | Configure name, type, unit, transform | Operator | ~30 seconds |
...
## 23. Migration & Upgrade Path (v5.1 → v5.2)

> **Status:** NEW in v5.2
> **Purpose:** Step-by-step migration guide for upgrading from v5.1 to v5.2.

### 23.1 Key Migration Steps

| # | Module | Change | Breaking? | Effort |
|---|--------|--------|-----------|--------|
| 1 | **M1** | Replace `StandardTelemetry` flat interface with flexible envelope (Business Trilogy) | Yes — downstream consumers must update | Medium |
| 2 | **M1** | Add `translation-executor.ts` — AppConfig-driven parser rule engine | No — new file | Low |
| 3 | **M8** | Add `vpp-data-dictionary` DynamoDB table (new CDK resource) | No — additive | Low |
| 4 | **M8** | Add `vpp-m1-parser-rules` AppConfig Configuration Profile | No — additive | Low |
| 5 | **M8** | Add `publishNewField()` atomic dual-write Lambda | No — new handler | Medium |
| 6 | **M2, M3, M4, M5** | Update downstream consumers to read from `telemetry.metering.*` / `telemetry.status.*` / `telemetry.config.*` instead of flat fields | Yes — field access pattern changes | Medium |
| 7 | **AppConfig** | Seed `vpp-m1-parser-rules` profile with existing hardcoded field mappings | No — data migration | Low |

### 23.2 Detailed Steps

**Step 1: M1 StandardTelemetry Migration**

Replace the flat `StandardTelemetry` interface with the v5.2 flexible envelope:

```typescript
// Before (v5.1):
const soc = telemetry.battery_soc;
const power = telemetry.grid_power_kw;

// After (v5.2):
const soc = telemetry.status['status.battery_soc'];
const power = telemetry.metering['metering.grid_power_kw'];
```

All downstream consumers (M2, M3, M4, M5) must update their field access patterns accordingly.

**Step 2: Seed AppConfig Parser Rules**

Convert existing hardcoded field mappings into the AppConfig `vpp-m1-parser-rules` JSON format. For example, the existing `HuaweiAdapter` becomes:

```json
{
  "version": "5.2",
  "rules": [
    { "domain": "metering", "targetField": "metering.grid_power_kw", "sourcePath": "dataItemMap.active_power", "transform": "divide:1000", "unit": "kW" },
    { "domain": "status", "targetField": "status.battery_soc", "sourcePath": "dataItemMap.battery_soc", "transform": "identity", "unit": "%" },
    { "domain": "metering", "targetField": "metering.voltage_v", "sourcePath": "dataItemMap.mppt_voltage", "transform": "identity", "unit": "V" },
    { "domain": "metering", "targetField": "metering.current_a", "sourcePath": "dataItemMap.mppt_current", "transform": "identity", "unit": "A" }
  ]
}
```

**Step 3: Deploy DynamoDB Data Dictionary Table**

Add the `vpp-data-dictionary` table to `AdminControlPlaneStack`:

```typescript
const dataDictionaryTable = new dynamodb.Table(this, 'DataDictionary', {
  tableName: 'vpp-data-dictionary',
  partitionKey: { name: 'orgId', type: dynamodb.AttributeType.STRING },
  sortKey: { name: 'fieldId', type: dynamodb.AttributeType.STRING },
  billingMode: dynamodb.BillingMode.PAY_PER_REQUEST,
  removalPolicy: cdk.RemovalPolicy.RETAIN,
});

dataDictionaryTable.addGlobalSecondaryIndex({
...
**Step 4: Seed Data Dictionary**

Backfill the `vpp-data-dictionary` table with entries for all existing fields:

```typescript
const seedEntries: DataDictionaryEntry[] = [
  { fieldId: 'metering.grid_power_kw', domain: 'metering', displayName: 'Grid Power', unit: 'kW', valueType: 'number', sourcePath: 'dataItemMap.active_power', transform: 'divide:1000', createdBy: 'migration-v5.2', createdAt: new Date().toISOString(), orgId: 'GLOBAL' },
  { fieldId: 'status.battery_soc', domain: 'status', displayName: 'Battery SoC', unit: '%', valueType: 'number', sourcePath: 'dataItemMap.battery_soc', transform: 'identity', createdBy: 'migration-v5.2', createdAt: new Date().toISOString(), orgId: 'GLOBAL' },
  // ... all existing fields
];
```
...
- **EventBridge event schemas:** No breaking changes — existing `ConfigUpdated` events continue to work. The new `SchemaEvolved` event is additive
- **Existing Lambda functions:** No breaking changes to M3, M6, M7 — they do not consume `StandardTelemetry` directly
- **v5.1 tests remain valid:** The output `StandardTelemetry` fields are semantically identical, just restructured into domain buckets (metering/status/config)
- **Data backfill:** Not required — M1 is a stateless translator. Historical data in Timestream is unaffected; new data flows through the flexible envelope automatically

### 23.4 Rollback Plan

If v5.2 causes issues in production:

1. **AppConfig rollback:** One-click rollback to previous `vpp-m1-parser-rules` version in AWS Console
2. **M1 Tier 3 fallback:** If AppConfig is unavailable, M1 automatically falls back to emitting raw payloads (§4.4)
3. **Code rollback:** Revert `StandardTelemetry` interface to v5.1 flat format; re-deploy M1-M5 Lambdas
4. **Data Dictionary:** `vpp-data-dictionary` DynamoDB table can be left in place (no harm); `SchemaEvolved` events are ignored by v5.1 consumers
